...
from tools._util import get_project_root_path
...
def _test_pyinstaller() -> str:
    ...
    if output_path.exists():
        logging.info("Removing %s ...", output_path)
    ...
    logging.info("Opening %s ...", app_path)
    try:
        ...
        logging.info("Re running: %s ...", command)
        subprocess.run(command)
        logging.info("Opening %s ...", app_path)
        result = subprocess.check_output([str(app_path)], stderr=subprocess.STDOUT)
...
def _check_output(output: str) -> None:
    if output == "":
        logging.info("There is no output from demo app")
        return
    ...
    for message in _IGNORE_MESSAGES:
        if message in output:
            logging.info("Ignore: %s", message)
            output = output.replace(message, "")
            continue
        logging.info("Demo app error: %s", message)
    if output.replace("\n", "") == "":
        return
...
def main() -> None:
    ...
    logging.info("PyInstaller test session start")
